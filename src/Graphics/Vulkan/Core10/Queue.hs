...
instance Show VkPipelineStageFlagBits where
  ...
  showsPrec _ (VkPipelineStageFlagBits 0x00200000) = showString "VK_PIPELINE_STAGE_RESERVED_21_BIT_NV"
  showsPrec _ (VkPipelineStageFlagBits 0x00080000) = showString "VK_PIPELINE_STAGE_RESERVED_19_BIT_NV"
  showsPrec _ (VkPipelineStageFlagBits 0x00100000) = showString "VK_PIPELINE_STAGE_RESERVED_20_BIT_NV"
...
instance Read VkPipelineStageFlagBits where
  readPrec = parens ( choose [ ("VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT",                    pure VK_PIPELINE_STAGE_TOP_OF_PIPE_BIT)
                             ...
                             , ("VK_PIPELINE_STAGE_RESERVED_21_BIT_NV",            pure (VkPipelineStageFlagBits 0x00200000))
                             , ("VK_PIPELINE_STAGE_RESERVED_19_BIT_NV",            pure (VkPipelineStageFlagBits 0x00080000))
                             , ("VK_PIPELINE_STAGE_RESERVED_20_BIT_NV",            pure (VkPipelineStageFlagBits 0x00100000))
                             ] +++
                      prec 10 (do
...
-- 'Graphics.Vulkan.Core10.SparseResourceMemoryManagement.vkQueueBindSparse',
-- 'vkQueueSubmit', 'Graphics.Vulkan.Core10.Fence.vkResetFences',
-- 'Graphics.Vulkan.Core10.Fence.vkWaitForFences'
newtype VkFence = VkFence Word64
  deriving (Eq, Show)

instance Storable VkFence where
  sizeOf (VkFence w) = sizeOf w
  alignment (VkFence w) = alignment w
  peek ptr = VkFence <$> peek (castPtr ptr)
  poke ptr (VkFence w) = poke (castPtr ptr) w
...
instance Storable VkSemaphore where
  sizeOf (VkSemaphore w) = sizeOf w
  alignment (VkSemaphore w) = alignment w
  peek ptr = VkSemaphore <$> peek (castPtr ptr)
  poke ptr (VkSemaphore w) = poke (castPtr ptr) w
...
--
-- -   If any element of @pSubmits@→@pCommandBuffers@ includes a [Queue
--     Family Transfer Acquire
...
--     there /must/ exist a previously submitted [Queue Family Transfer
--     Release
...
--     definition of such [acquire
--     operations](https://www.khronos.org/registry/vulkan/specs/1.0-extensions/html/vkspec.html#synchronization-queue-transfers-acquire),
